(function($) {
		...
		//Send the delete
			...
			//Send the process request to the server
				$.ajax({
					...
					'type' : 'POST',
					'url' : document.location.href.substring(0, document.location.href.indexOf('travel-assistant')) + 'wp-content/plugins/travel-assistant/app/system/ajax/delete.php',
					...
					//If the transaction was successful, close the dialog
						...
						//Decrement the total trip counter near the top of the page
							...
							if (button.parent().parent().hasClass('shared')) {
								...
								value == 1 ? total.next('span').text('Trip I\'ve Shared') : total.next('span').text('Trips I\'ve Shared');
							}
	...
	//Instantiate the visited plugin
		$('article.map').FFI_TA_Visited();
	});
})(jQuery);